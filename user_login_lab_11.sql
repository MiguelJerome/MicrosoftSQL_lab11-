...
-- This will display a list of all the users, along with their type (either Windows login or SQL login) and authentication type (either Windows authentication or SQL Server authentication). Note that this query only shows the server-level principals and not the database-level principals. To see the database users, you can execute a similar query on the specific database:
